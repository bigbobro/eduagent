...
export class LessonController {
  ...
  private fillerPcm: ArrayBuffer | null = null;
  ...
  // Pre-rendered "thinking" filler (24kHz int16 PCM) replayed locally to mask LLM-first-token dead
  // air. Generated by scripts/gen-filler.ts in the current TTS voice.
  private static readonly FILLER_AUDIO_URL = '/audio/thinking-filler.pcm';
  ...
  async startLesson(courseId: string): Promise<boolean> {
    ...
    await Promise.all([
      ...
      this.loadFiller().catch(() => {}), // best-effort; no filler = no lesson impact
    ]);
  ...
  private async finishListening(stoppedAt: number): Promise<void> {
    ...
    // Mask the ~4-6s LLM-first-token dead air with a short, content-free filler. Local PCM enqueue
    // (no TTS session — that lifecycle is fragile, see reliability work), gated to LLM turns so
    // reinforcement repeat-after-me (routeToChat:false) stays silent.
    if (this.fillerPcm && this.routeCurrentAsrToChat) this.player.enqueue(this.fillerPcm);
  ...
  // Best-effort: fetch the pre-rendered thinking filler once at lesson start. Cached for the
  // controller's lifetime and replayed locally (PcmPlayer) on each LLM-turn thinking entry.
  private async loadFiller(): Promise<void> {
    if (this.fillerPcm) return;
    const res = await fetch(LessonController.FILLER_AUDIO_URL);
    if (!res.ok) return;
    this.fillerPcm = await res.arrayBuffer();
  }
