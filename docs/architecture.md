...
- **repeat-after-me 句子候选注入(2026-07-03)**:sports 真人课(session `1b096ae7`)quiz 轮 "I play tennis." 被识别成 "I'll play Thomas."——热词从不含句子是缺口。现在 `ReinforceFrame` 把当前 quiz 的 `targetText`(在前)+ 同组其余 repeat-after-me 句(次级候选)经 `startListening({routeToChat:false, asrSentenceTexts})` → `setAsrSessionContext` → WS query `sentenceText`(每句一个 param,整值保留、不做逗号切分)传给 proxy;`buildHotWords` 把句子候选放在最前,后接原有整课词表 fallback(2026-06-21 行为不变),word 卡轮完全零回归(不带 `sentenceTexts` 时热词构造逐字节不变)。**注入格式采用"整句 + 内容词"双注入**(R4 决策):本地协议摘要没有条目长度/格式限制与 per-entry weight,故整句(去尾部标点)作短语级候选,同时拆 ≥3 字母的内容词(与 `ReinforceFrame` 判分 token 切法一致)——即使供应商忽略多词短语条目,判分实际 grep 的词(play/tennis)也各自获得偏置。直连探针(TTS 念 "I play tennis." → ASR)实证:整句(含空格)hotword 条目豆包不报协议错、识别命中;儿童真实发音效果待下节课实测。
