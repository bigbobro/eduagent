...
- 开发调试面板仅在 `NODE_ENV=development` 显示;除强制 phase 跳转外,`skip word` 会调用 `/api/chat?action=debug-skip-word`,把当前 session 的当前 word card 标记为 `cleared` 并直接 `show_card` 下一张。该操作只改本节课的 session-local `clearedCardIds/cardProgress`,用于调试流程,不代表 durable mastery。
