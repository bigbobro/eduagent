...
| | | | | | |\
| | | | | | |\
| | | | | |EduAgent Gotcha: 热词直传不是顶层 `hot_words` 字段,而是 `request.corpus.context` 里的 JSON string,内部字段名是 `hotwords`。当前协议原文未给每个 hotword 提供 weight 字段;`language_hint` 字段也未出现。当前 ASR proxy 继续不传 `audio.language`,使用文档说明的默认中英文识别能力。 |\
| | | | | | |\
