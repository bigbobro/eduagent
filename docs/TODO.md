...
- 2026-06-21 — **同音词内容 pass**(`scripts/asr-word-scan.ts` TTS→ASR 全量探针,480 词):17 词标准发音被 ASR 听错,补 16 个 `asrAliases`(gray→grey / aunt→ant / star→store ×3 / harp→hop / sink→sing / skating→gating / toolbox→true box / ladder→letter / canvas→candice / speaker→speak / fork→fool / kneepads→neepads / suitcase→seatcase / yoyo→悠悠;knight 已先修)。bee 复测 HIT 3/3 稳定,不加。CLAUDE.md 加造课 checklist。**待办:`leg`/`spell`/`palette`/`stamp` 探针连续超时(ASR 未返回 final,同批 bee 正常→词特异性),拿不到输出无法定 alias,需单独重跑 `pnpm tsx scripts/asr-word-scan.ts --cards body:leg,magic:spell,art-supplies:palette,art-supplies:stamp` 排查。**
