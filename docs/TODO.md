...
## 0. ~~下次迭代目标:画布与教学策略重新设计~~ ✅ (2026-05-05 完成)

> **已完成。** v2 词卡画布方案落地:协议统一为 `show_card`,数据迁移到 `cards[]`,画布改为图+中英文独立 DOM 层。详见 `docs/architecture.md §9` 及 `docs/superpowers/plans/2026-05-05-canvas-v2-wordcard.md`。
>
> 原始问题描述(已解决):
...
**要做的前置思考(plan 之前)** — ✅ 全部已在 v2 方案中解决:

- [x] 教学场景分析 → v2 方案:统一 `show_card` 协议,LLM 通过 `teachingHints` 控制卡片顺序
- [x] 数据结构评估 → 已迁移到 `cards[]` + `teachingHints`,两个课程文件均完成
- [x] 画布分层 → WordCardCanvas:图片层 + 中英文 DOM 层独立渲染,`data-kind` 区分 word/sentence
- [x] 文本层方案 → 静态写在课程 JSON(`WordCard.english/chinese`),画布 DOM 独立渲染
- [x] ~~bbox~~ → v2 方案不再需要 bbox,用独立中英文 DOM 层替代图片内嵌文字
...
## 3. 画布交互优化(v2 WordCardCanvas)

- [ ] **卡片切换动画** — `show_card` 切换时的过渡动画可以更生动(缩放+淡入,而不是简单 opacity fade)
- [ ] **中文字体样式** — 当前用默认 sans-serif,可换成更适合儿童的圆体/手写体
- [ ] **图片加载态** — 新卡片图片加载中时显示 loading skeleton,避免白屏闪烁
...
| 优先级 | 任务 | 原因 |
|--------|------|------|
| ~~P0~~ ✅ | ~~画布与教学策略重新设计(§0)~~ | v2 词卡画布已落地(2026-05-05) |
