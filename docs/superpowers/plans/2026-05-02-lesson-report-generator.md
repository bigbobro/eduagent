...
- [x] **Step 1: 写测试 helper(seed 假课到 in-memory DB)**
...
- [x] **Step 4: 实现最小 buildReport — 基础聚合**
...
- [x] **Step 1: 写 highAvgInput 测试(false 与 true 两 case)**
...
- [x] **Step 5: 写 asr/tts/tokensCorrupted 测试**
...
- [x] **Step 6: 跑测试确认 asr/tts 已 PASS,tokensCorrupted FAIL**

Run: `pnpm test scripts/lesson-report-data.test.ts`
Expected: asr/tts 两条 PASS(已经从 `tracked` 派生),tokensCorrupted FAIL(flag 永远 false)。

- [x] **Step 7: 实现 tokensCorrupted + asr/tts not tracked flag**
...
- [x] **Step 3: 加 default courseLoader + main 入口**
...
- [x] **Step 4: 检查 main 不污染单测(单测里不应触发 main 执行)**
...
- [x] **Step 1: architecture.md 加"开发工具链 — 课后报告"节**
...
- [x] **Step 2: TODO.md 加报告引用**

在 `docs/TODO.md` §0 那条 "**课程反馈分析(2026-05-02 transportation 课观察)**" 项目末尾加一行,变成:

```markdown
- [x] **课程反馈分析(2026-05-02 transportation 课观察)** — session `cf63ef96-5dd1-4f40-89a7-ad324a311680`,...(原内容不动)
...
- [x] **Step 4: Claude 内跑指定 session(用一节边界 case 课)**
...
- [x] `pnpm test` 全部通过(原 27 个 + 本 plan 新增 ~13 个 ≈ 40 个测试)
- [x] `pnpm exec tsc --noEmit` 无错
- [x] `pnpm tsx scripts/lesson-report-data.ts`(无参)能输出有效 JSON,字段含 `session.targetWords` 非空
- [x] `pnpm tsx scripts/lesson-report-data.ts not-a-real-id` exit 1,stderr 有可读错误 + 最近 5 节列表
- [x] Claude 内 `/lesson-report` 跑通,在 `docs/lesson-reports/` 落档一份 markdown,文件名日期来自 session.startTime(不是今天)
- [x] 该 markdown 文件被 `.gitignore` 忽略(`git status` 不显示),`.gitkeep` 已被 commit
- [x] `docs/architecture.md` 末尾有"开发工具链 — 课后报告生成器"节
- [x] `docs/TODO.md` §0 课程反馈条目下有引用 `/lesson-report` 的一行
- [x] 9 个 task 各自产生独立 commit(linear,无 amend / 无 force push)
- [x] 本 plan 所有 `- [ ]` checkbox 全部改成 `- [x]`
