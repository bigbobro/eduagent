...
- Old `src/components/bunny` and `src/components/scene` directories are removed from the working tree.
...
- `git ls-files src/components/bunny src/components/scene` will only return zero after the deletion is staged or committed; the files are already deleted in the working tree.
